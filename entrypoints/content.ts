export default defineContentScript({
  ...
  async main() {
    ...
    const extractVideoInfo = (anchor: HTMLAnchorElement, container: Element): Video => {
      ...
      let subchannel = '';
      const subchannelEl =
        container.querySelector('a[class*="channelName"]') || container.querySelector('[class*="channelName"]');
    ...
    const processVideoLinks = (): void => {
      ...
      links.forEach((anchor) => {
        ...
        const hasImage = anchor.querySelector('img') || anchor.querySelector('[style*="background-image"]');
        if (!hasImage) return;
